...
public class Secretary(private val connection: Connection) {
    ...
    public data class InboxItem(val taskId: UUID, val kind: String, val reason: String, val dueAt: Instant?, val state: String, val section: InboxSection,
                                ...
                                /**
                                 * The team a task belongs to, when it has one (PD-142).
                                 *
                                 * The phone drew a card for a result that is owed with nothing to tap on it: the task
                                 * said which fixture by name and carried no way to reach it. The team is the way — its
                                 * page lists the fixture — and the id is already on the row.
                                 */
                                val team: UUID? = null)
    ...
    private fun inboxWhere(where: String, id: UUID, now: Instant): Map<InboxSection, List<InboxItem>> {
        ...
        connection.prepareStatement(
            "SELECT task_id, kind, reason, due_at, state, subject_player_id, "
                + "CASE WHEN source_kind = 'rearrangement_proposal' THEN source_id END, owner_team_id "
                + "FROM competition.admin_task WHERE $where ORDER BY due_at NULLS LAST, created_at",
        ).use { ps ->
            ps.setObject(1, id)
            ps.executeQuery().use { rs ->
                while (rs.next()) {
                    ...
                    items += InboxItem(rs.getObject(1) as UUID, rs.getString(2), rs.getString(3), due, rs.getString(5),
                                       Inbox.sectionOf(state, due, now, endOfToday),
