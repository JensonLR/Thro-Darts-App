...
| On-device journal, Android | 32 tests — ADR-006's journal in Kotlin, held to the **same claims** as the iOS one rather than translated from it: the configuration read back on open, append-only by trigger, replay through the Kotlin engine that throws on a corrupt row and on a row kind this build cannot read, a retraction that supersedes and never deletes, endings that are final, attestation that changes no score and goes stale when the result moves under it, the shelf and the one narrowed delete (PD-026), and the device identity a journal keeps when the caller forgets it. Two hold what the platforms owe each other: **the stored vocabulary** — the `kind` and `seat` words a Kotlin enum must serialise to — and the timestamp format, because a row written on an iPhone is meant to be readable on a Pixel. **No Android durability number is claimed**: Apple's `fullfsync` has no Android equivalent, setting the pragma there would read back as 1 while nothing had happened to the hardware, and a test holds the configuration to the two pragmas that can honestly be verified so the shape of that decision cannot be widened into a claim |
