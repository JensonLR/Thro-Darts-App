...
public struct ThroWidgetBoard: View {
    ...
    private func scoreboard(_ live: ThroLiveState, _ projection: ThroProjection) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if !caption.isEmpty {
                Text(caption)
                    .font(ThroTypography.metadata.weight(projection.isStale(now: now) ? .semibold : .regular).fixed(wide ? 13 : 11))
    ...
    private func side(_ name: String, _ remaining: Int, _ legs: Int, throwing: Bool) -> some View {
        VStack(alignment: .leading, spacing: 1) {
            HStack(spacing: 4) {
                ...
                Text(name)
                    .font(ThroTypography.label.weight(throwing ? .semibold : .regular).fixed(wide ? 13 : 11))
            ...
            HStack(alignment: .firstTextBaseline, spacing: 5) {
                Text("\(remaining)")
                    .font(ThroTypography.sportHero.fixed(wide ? 40 : 32))
                    .monospacedDigit()
                    .foregroundStyle(ThroColor.throChalk)
                    .lineLimit(1)
                    .minimumScaleFactor(0.6)
                Text("\(legs)")
                    .font(ThroTypography.sportHero.weight(.semibold).fixed(wide ? 15 : 13))
                    .monospacedDigit()
    ...
    private var waiting: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let projection, let (title, detail) = ThroWidgetCopy.fixture(projection, now: now) {
                Text(title)
                    .font(ThroTypography.label.fixed(wide ? 15 : 13))
                    .foregroundStyle(ThroColor.throChalk.opacity(0.9))
                    .lineLimit(2)
                Text(detail)
                    .font(ThroTypography.metadata.fixed(wide ? 12 : 10))
