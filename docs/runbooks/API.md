...
| `POST /v1/teams/{teamId}/league` | the team's admin or captain | Say the team plays in a league THRØ lists (PD-049, V039). Carried as the team's own say: it reaches `/v1/leagues` as that league's `saidTeams`, never as one of its divisions' teams. 404 when THRØ lists no such league; saying it twice is saying it once |
| `DELETE /v1/teams/{teamId}/league/{leagueId}` | the team's admin or captain | Stop saying it. The claim is marked withdrawn and kept, so who said it and when stays readable; saying it again later is a new claim |
...
| `GET /v1/seasons/{leagueSeasonId}/fixtures` | anyone | A season's fixtures, soonest first (PD-056): the date, the lifecycle (scheduled, rearranged, postponed), the venue, and what each finished as where it has — played, declared by an official, awarded or walked over. Read from the same rows the table is, so the two cannot disagree. A private team or venue is **not named**, and the fixture is still listed, because hiding it would leave a hole in the league's own calendar |
...
| `GET /v1/streams/match/{matchId}` | a participant — including a player who took their seat with a code (PD-043) — the holder of a grant in force, or an official of the event | `text/event-stream`: every event of the match in commit order, each carrying its own `eventId` so a correction names a row the reader holds, then each new one — woken by the database the moment one commits (V036), and still polling once a second beneath that; `Last-Event-ID` resumes; a comment ping every 15 s; the client treats 45 quiet seconds as stale; who may watch is asked again every minute, so a revoked grant or an ended session closes the stream (ADR-007) |
...
condition, waiting on the organiser console). Watching by anybody but a match's two players, its
grant holders and its officials (PD-044): the spectator stream, filtered for such a reader, is
ADR-007's and not built.
