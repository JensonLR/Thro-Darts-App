...
> 699 tests in all: 169 design, 105 journal, 99 scoring session and share card, 301 opening, app and teams, 25 Lock Screen, wall and widgets — and the
...
leaves the phone, and only when somebody sends it**: a finished match (Live → Send to THRØ, PD-040),
ending and all (PD-042), as the journal wrote it. The other player then takes their seat with a code
...
attestation, no rating (OD-001), no organiser surface, and nothing live from anybody else — watching a
match from another phone is next, and the Live tab says so. **Sign-in exists** (Settings → Account and profile): Sign in with Apple, Sign in with Google once the build carries a client id, and a passkey — against the staging server, with THRØ's own session kept in the keychain; from there the person's Secretary inbox and the events they can play are read from the server with their reasons. Signing in uploads nothing. Clubs exist on the device only:
