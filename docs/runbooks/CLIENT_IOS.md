...
8. The home-screen icon: the mark in chalk on green. The launch screen: the mark with THRØ beneath it,
   chalk on green. **Is the wordmark right?** Its letters are set from Archivo ExtraBold on the reading
   that the supplied wordmark is that face; say if it is not.
...
The app icon is the mark in chalk on the brand green; the launch screen is the mark with the wordmark
beneath it, as the export's Splash screen composes them. The icon is generated by
`docs/design/brand/render_mark.py` from geometry measured against the founder's artwork; the launch
image by `docs/design/brand/render_wordmark.py`, which sets THR from Archivo ExtraBold's own outlines
and draws the Ø from the mark's geometry at the wordmark's heavier proportions. That face is a
judgement from the supplied image — the forms match, its letters run a few percent narrower — and the
founder confirms or corrects it on the phone. `docs/design/brand/README.md` says how to replace both
with the master files when those are added.
