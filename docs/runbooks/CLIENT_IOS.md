...
8. The home-screen icon: the mark in chalk on green. Then the opening, fourth version: a pool of light
   far up the wall; a dart flying in profile with the camera alongside it — the chalk dust streaming
   past, the frame rolling upright, the light rushing in — and the hit: the thud and the heavy buzz, the
   frame shaking, dust, the flights whipping while the barrel stays still; four strokes of chalk drawing
   the ring where the light was, with the chalk sound; the mark sliding into the last slot of THRØ as
   T, H, R stamp in beside it, a firm tap each; the tagline beneath; Home fading up — four and three-quarter
   seconds. Silent switch on: no sound. A tap goes straight to Home. Settings → Opening → *Play the
   opening again* replays it. **Does the dart feel thrown, and does the hit land with purpose? Does the
   name arrive right, and is the pace right? Do the sounds fit?** Each of the three sounds can be replaced
   by a recorded one under the same filename. **Is the wordmark right?** Its letters are Archivo
   ExtraBold on the reading that the supplied wordmark is that face; say if it is not. With Reduce Motion
   on, the name and the tagline, still.
...
alone. The app's first frames are the opening (PD-007, fourth version): a tracking shot. The camera
flies with a real dart — needle, barrel, shaft, flights — which holds in profile mid-frame, rolling,
nose-up and settling, while the wall comes to it: the chalk dust streams past, the camera rolls from a
flat throw to the mark's angle, and a pool of light — the dart's destination, far and small — grows as
an approaching thing does and arrives at the strike. Thud, a heavy haptic, the light breathing, the frame
shaking, dust, the dart squashing for a few frames and its flights whipping while the barrel stays still.
Four strokes of chalk run round where the light was until the ring is whole. Then the mark shrinks and
slides into the last slot of THRØ, taking the letters' weight, as T, H, R stamp in beside it with a firm
haptic each; the tagline tracks in beneath; Home fades up. Four and three-quarter seconds, once per cold
