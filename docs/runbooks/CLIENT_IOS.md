...
8. The home-screen icon: the mark in chalk on green. Then the opening: green field, one chalk dart
   from the lower left, once round leaving the ring, straight through, a flex on landing, the mark
   rising to its place with THRØ and the tagline beneath, Home fading up — about two seconds. A tap
   during it goes straight to Home. **Is the wordmark right?** Its letters are Archivo ExtraBold on the
   reading that the supplied wordmark is that face; say if it is not. **Does the motion feel right?**
   It was judged from six still frames; the phone is the first place it moves.
...
alone. The app's first frames are the opening (PD-007): one chalk dart comes in from the lower left,
loops once leaving the ring as its trail, cuts through the centre and lands — its path is the mark —
then the mark rises to its place and THRØ appears beneath it, the export's Splash composition, and
Home fades up. It takes 2.3 seconds, plays once per cold launch, and a tap skips it; with Reduce
Motion on it shows the finished composition and fades. The wordmark's letters are Archivo ExtraBold,
drawn live, on the reading that the supplied wordmark is that face — the forms match, its letters run a
few percent narrower — which the founder confirms or corrects on the phone. The storyboard and spec are
the design canvas "THRØ Launch Sequence"; `docs/design/brand/README.md` says how to replace the mark
with the master file when it is added.
