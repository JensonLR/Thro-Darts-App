...
| `packages/client-ios` → `ThroApp` | Home, tabs, You, Settings, the root view, the opening (PD-007) | eleven tests on the opening's timeline, cues, easings, geometry, flight path, chalk stroke and fold on macOS, every push; the screens are drawn, not tested |
...
8. The home-screen icon: the mark in chalk on green. Then the opening, fifth version: a stage light far
   up the wall; a real dart caught mid-flight, resolving out of a smear, flying in profile at one angle
   with the camera alongside it — the chalk dust streaming past and lit in the beam, the light rushing
   in — and the hit: the thud and the heavy buzz, the frame shaking, dust, the flights whipping while
   the barrel stays still; four strokes of chalk drawing the ring where the light was, with the chalk
   sound; the mark sliding into the last slot of THRØ as T, H, R stamp in beside it, a firm tap each;
   the tagline beneath; Home fading up — four and three-quarter seconds. Silent switch on: no sound. A
   tap goes straight to Home. Settings → Opening → *Play the opening again* replays it. **Is the angle
   right now, all the way? Does the dart look like a dart up close — the barrel, the flights with the
   mark on them? Does the hit land? Is the pace right? Do the sounds fit?** Each of the three sounds can
   be replaced by a recorded one under the same filename. **Is the wordmark right?** Its letters are
   Archivo ExtraBold on the reading that the supplied wordmark is that face; say if it is not. With
   Reduce Motion on, the name and the tagline, still.
...
alone. The app's first frames are the opening (PD-007, fifth version): a tracking shot. The camera
catches a real dart mid-flight — a fine point, a tapered barrel with grip rings, a collar, a slim shaft,
standard flights carrying the mark — which resolves out of a motion smear and holds in profile
mid-frame at the mark's angle, rolling, while the wall comes to it: the chalk dust streams past, lit
where a stage light's beam falls, and the pool of light where the dart is going grows as an approaching
thing does and arrives at the strike. Thud, a heavy haptic, the light breathing, the frame shaking,
dust, the dart squashing for a few frames and its flights whipping while the barrel stays still. Four
strokes of chalk run round where the light was until the ring is whole. Then the mark shrinks and slides
into the last slot of THRØ, taking the letters' weight, as T, H, R stamp in beside it with a firm haptic
each; the tagline tracks in beneath; Home fades up. Four and three-quarter seconds, once per cold
launch; a tap skips it. Sound plays through the ambient session, so the silent switch silences it;
Settings → Opening has switches for the sound and the haptics and *Play the opening again*. With Reduce
Motion on it shows the name and the tagline and fades, silently, with nothing moving. The
