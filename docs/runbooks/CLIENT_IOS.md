...
> 958 tests in all: 196 design, 107 journal, 104 scoring session and share card, 471 opening, app and teams, 25 Lock Screen, wall and widgets, 29 on a wrist, 26 on a wall — and the
...
**The notice about people's information** (PD-094): add `-ThroWebBaseURL http://localhost:8787` to read it from a
local copy of `apps/web` (`python3 -m http.server 8787 --directory <copy>`), and set `"active": true` with every
field filled in that copy's `notice.json` — `docs/legal/BREACH_PLAN.md` has the shape. The card appears on Home.
...
#### Putting it on an Apple TV

The Apple TV app builds and runs in the simulator with no account at all. A **real** Apple TV needs the
device registered to the team first, and `xcodebuild` cannot do that part: it will happily create the App
ID, but a *development* provisioning profile for a platform with no device on it cannot exist, which is why
a device build fails with *"No profiles for 'app.thro.darts' were found"* even though the phone signs fine.

Recent Apple TVs have no port to plug into, so pairing is over the network and both machines must be on the
same Wi-Fi:

1. On the Apple TV: **Settings → Remotes and Devices → Remote App and Devices**. Leave that screen open —
   it is listening only while it is showing.
2. On the Mac: **Xcode → Window → Devices and Simulators → Devices**. The Apple TV appears in the left-hand
   list within a few seconds, marked as needing to be paired.
3. Click **Pair**. The Apple TV shows a six-digit code; type it into Xcode.
4. Xcode registers the device with your team and prepares it, which takes a minute or two the first time
   (it copies debug symbols across).

After that, `ThroTV` runs from Xcode like any other target, and the command line works too:

```bash
xcodebuild -project apps/ios/ThroDarts.xcodeproj -scheme ThroTV -configuration Debug -destination 'generic/platform=tvOS' -allowProvisioningUpdates build
```

**The bundle identifier is deliberately the same as the phone's** — `app.thro.darts`. That is what makes one
App Store listing cover iPhone, iPad and Apple TV as a single purchase, so there is no second product page
to maintain and no second price. Two targets sharing an identifier is fine because they are different
platforms.

**What is not done yet** is the artwork: tvOS wants a layered parallax app icon (a 5:3 image stack) and two
top-shelf images, and there is no automatic way to make good ones from the square phone icon. The app builds
and runs without them; the store will not take it without them.
...
- **A normal visit.** The keypad is a row of **SINGLE · DOUBLE · TREBLE**, then the twenty sectors
  **counting down from 20** (PD-034), `20 19 18 17 16` first and `5 4 3 2 1` last — then **25 · BULL · MISS ·
  ENTER**. Counting order needs no learning; it runs downwards so the numbers a scorer reads
  most are the first row, in the order a player says them. Tap **TREBLE**, then **20**: the key says *T20* before you commit to
...
- **On an iPhone SE**, in this notation and on a finish, the checkout card is **not** drawn, the
  ledger goes, and the score is **56 pt** where typing totals gives it 72: the board has 161 points,
  and its head, the three darts and the smallest number on the ladder come to more than that. The
  keys do not change and nothing scrolls. Deliberate, stated in `ThroStage` as an order of sacrifice
  — ledger, then the route, then the tray's comfort, never the number below the ladder and never a
  key under 44 — and asserted over 1,056 device / orientation / text-size / notation combinations.
  (On the same phone **off** a finish, typing totals, the score is still the top rung at 96 pt.)
...
- **Settings** (from the gear on You). **Appearance: System / Light / Dark** — every screen follows
  ...
  built from, that matches are kept on this phone, that a match is sent to THRØ from the Live tab,
  and which face the fonts are.
...
journal refuses to open under any other configuration. Deleting the app deletes the journal. The
THRØ session, when you have signed in, is one item in the app's own keychain (this device only,
never restored to another phone); nothing else about you is stored, and no match is uploaded.
...
No sync: the journal on the phone is the record of what it scored, and nothing the server says
changes it. The network target, `ThroNet`, is reached only by `ThroApp`, never by the journal or the
scoring session, which is how LATENCY_BUDGETS.md's structural requirement is enforced. **One thing
leaves the phone, and only when its player asks**: a match's journal, as it wrote it — sent once the
match is finished (Live → Send to THRØ, PD-040), ending and all (PD-042), or shared as it is scored
(PD-044). The other player then takes their seat with a code
the sender makes, and confirms the result or contests it (Live → On THRØ, PD-043); what the Live tab
shows of a sent match is the server's reading of it, and the phone keeps none of it as truth. No
attestation, no rating (OD-001), no organiser surface, and nothing live from anybody but the other
player of a match they share with you as they score it (Live → On THRØ, PD-044) — the page of a match
still going is its board, replayed from the stream, and it says when the stream is not live. **Sign-in exists** (Settings → Account and profile): Sign in with Apple, Sign in with Google once the build carries a client id, and a passkey — against the staging server, with THRØ's own session kept in the keychain; from there the person's Secretary inbox and the events they can play are read from the server with their reasons. Signing in uploads nothing. Clubs exist on the device only:
nothing is published, no club can be joined or searched for, and no announcement can be sent — the
composer exists and refuses, with the reason. (A team on THRØ is a different thing: started, and
joined by code, from Discover → Join or start, PD-036.) A player profile is reachable from a club's roster and
...
phone, lets them undo a mis-key, keeps it, and sends it when asked. That is all it claims.

## Looking at a screen that reads from the server

Most of THRØ's screens can be looked at with nothing behind them. A league's table and the fixtures beside
it cannot — they are the server's answer — and staging holds no seasons, so they were being changed without
anybody opening them. A **Debug** build takes the server as an argument:

```bash
xcrun simctl launch <device> app.thro.darts -ThroAPIBaseURL http://localhost:8080 -ThroScreen tab/discover
```

Run the API against a local Postgres with `PGHOST=localhost PGUSER=postgres PGDATABASE=postgres gradle -p
services/api serve` — note **`serve`**, not `run`, which starts the playtest harness and answers HTML to
every API path. Seed a season into it however you like; `StandingsTest`'s own inserts are the shape that
satisfies every trigger.

A Release build ignores the argument: it is compiled out, so a shipped app cannot be talked into pointing at
somebody else's server.

There is **no route to a league season**, so the table is reached by tapping: Discover → the league → TABLE.
ADR-011 fixes the public URL shapes and adding one for a season is a decision about public links, not a
convenience for a script.
