...
> 573 tests in all: 156 design, 100 journal, 95 scoring session and share card, 197 opening, app and clubs, 25 Lock Screen, wall and widgets — and the
...
| `packages/client-ios` → `ThroPlay` | setup, ready, scoring, result, undo, the bust and leg announcements, double-in (PD-008), the checkout route (PD-013), confirming the result (PD-011) and the share card | 95 session tests on macOS, every push |
