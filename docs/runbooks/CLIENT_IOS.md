...
> **Verification status, 2026-09-07.** Every package compiles and every test passes on macOS CI — 75
> tests: 20 design, 15 journal, 27 scoring session, 13 opening — and the Xcode app builds for the iOS simulator on
...
| `packages/client-ios` → `ThroApp` | Home, tabs, You, Settings, the root view, the opening (PD-007) | thirteen tests on the opening's timeline, the tagline's read time, cues, easings, geometry, the throw, the chalk stroke, the wall's dust and the dart on macOS, every push; the screens are drawn, not tested |
