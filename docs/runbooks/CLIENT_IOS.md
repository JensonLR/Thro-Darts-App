...
> 512 tests in all: 134 design, 100 journal, 73 scoring session and share card, 180 opening, app and clubs, 25 Lock Screen, wall and widgets — and the
...
| `packages/client-ios` → `ThroDesign` | the approved components as SwiftUI | 134 design tests on macOS, every push |
...
| `packages/client-ios` → `ThroPlay` | setup, ready, scoring, result, undo, the bust and leg announcements, double-in (PD-008), the checkout route (PD-013), confirming the result (PD-011) and the share card | 73 session tests on macOS, every push |
