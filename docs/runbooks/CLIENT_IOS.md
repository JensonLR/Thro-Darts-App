...
> 668 tests in all: 169 design, 105 journal, 99 scoring session and share card, 270 opening, app and teams, 25 Lock Screen, wall and widgets — and the
...
No sync and no upload of anything on the phone — the network target, `ThroNet`, is reached only by `ThroApp`, never by the journal or the scoring session, which is how
LATENCY_BUDGETS.md's structural requirement is enforced. No attestation, no rating (OD-001), no
organiser surface, and nothing live from anybody else — the Live tab shows this phone's own matches and its own clubs' unfinished fixtures, and says so. **Sign-in exists** (Settings → Account and profile): Sign in with Apple, Sign in with Google once the build carries a client id, and a passkey — against the staging server, with THRØ's own session kept in the keychain; from there the person's Secretary inbox and the events they can play are read from the server with their reasons. Nothing on the phone is uploaded by any of it. Clubs exist on the device only:
