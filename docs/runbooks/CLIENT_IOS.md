...
8. The home-screen icon: the mark in chalk on green. Then the opening, third version: a faint chalk
   ring waits on the green; a dart spins in from the lower left, cuts the ring twice — a light tick in
   your hand each time, the chalk lighting where it was cut — and strikes: the thud and the heavy buzz,
   the frame shaking, dust, the flights whipping and settling while the barrel stays still; four
   strokes of chalk run out from the cuts until the ring is whole, with the chalk sound; the dart
   folds into the mark's bar as the mark rises to its place; THRØ arrives beneath it letter by letter,
   the Ø drawing its own ring; Home fades up — four and a half seconds. Silent switch on: no sound. A
   tap goes straight to Home. Settings → Opening → *Play the opening again* replays it. **Does the
   strike land? Do tick, tick, thud feel like one gesture? Is the pace right? Do the sounds fit?** Each
   of the three sounds can be replaced by a recorded one under the same filename. **Is the wordmark
   right?** Its letters are Archivo ExtraBold on the reading that the supplied wordmark is that face;
   say if it is not. With Reduce Motion on, nothing should move at all.
...
alone. The app's first frames are the opening (PD-007, third version): a faint chalk ring waits on the
green as the target; a real dart — needle, barrel, shaft, flights — spins in from the lower left on an
arc, grows as it closes, cuts the ring twice (a light haptic tick each time, the chalk lit where it was
cut) and strikes: thud, a heavy haptic, the frame shaking, dust, a shockwave, the dart squashing for a
few frames and its flights whipping and settling while the barrel stays still; four strokes of chalk
run out from the cuts until the ring is whole, glowing while it blooms; the dart folds into the mark's
bar as the mark rises to its place and THRØ arrives beneath it letter by letter, the Ø's ring drawing
itself, the export's Splash composition; Home fades up. Four and a half seconds, once per cold launch;
a tap skips it. Sound plays through the ambient session, so the silent switch silences it; Settings →
Opening has switches for the sound and the haptics and *Play the opening again*. With Reduce Motion
on it shows the finished composition and fades, silently, with nothing moving. The
