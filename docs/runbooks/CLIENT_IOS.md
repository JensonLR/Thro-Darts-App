...
> 548 tests in all: 154 design, 100 journal, 89 scoring session and share card, 180 opening, app and clubs, 25 Lock Screen, wall and widgets — and the
...
| `packages/client-ios` → `ThroPlay` | setup, ready, scoring, result, undo, the bust and leg announcements, double-in (PD-008), the checkout route (PD-013), confirming the result (PD-011) and the share card | 89 session tests on macOS, every push |
