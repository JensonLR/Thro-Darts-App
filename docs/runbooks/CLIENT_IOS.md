...
> 943 tests in all: 193 design, 105 journal, 107 scoring session and share card, 458 opening, app and teams, 25 Lock Screen, wall and widgets, 29 on a wrist, 26 on a wall — and the
...
| `packages/client-ios` → `ThroPlay` | setup, ready, scoring, result, undo, the bust and leg announcements, double-in (PD-008), the checkout route (PD-013), confirming the result (PD-011) and the share card | 107 session tests on macOS, every push |
