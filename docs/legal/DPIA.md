...
6. **Art 28 processor terms** with Render, Neon, the mail host and — before `THRO_TYPESAFE_API_KEY` is set — TypeSafe. **The TypeSafe condition was crossed, not met** (noted 18 Sep 2026): the key is set on Render and production has been reading reports and chosen names since 11 September. It is tracked in `ROPA.md` now; either obtain the terms or unset the key, which degrades safely (PD-118: it is sent the words of a report and a chosen name, never who wrote them); **Art 27 EU representative** if EU users
   are targeted.
