...
**Date:** 2026-09-10 · **Status:** Phases A, B2, D, E's read model, C's store and the HTTP layer delivered; FB-1 and FB-2 decided (PD-030, PD-031); B3 delivered under §12d (Apple/Google sign-in, sessions, passkeys) and **staging is live** on Neon London + Render (2026-09-10); **the phone signs in** (Apple, Google, passkeys via `ThroNet` and the Account screen, with the Secretary inbox and discovery read from the server); C's surface, E's surface and F follow · **Precedence:** rank 4 (product/domain
...
| 21 | The HTTP layer: identity from the principal and never the body; a stranger with a good seat is refused before evidence exists; replays return the stored answer with its status for visits and organisational commands alike; bodies are bounded (64 KiB, 32-deep JSON, 32-player lineups) and hostile input is a 400, never a 500; the served contract equals the committed one; the development authenticator cannot be constructed without `THRO_DEV_AUTH=1` against a local database | `HttpTest` (27 properties), `InjectionTest`, `GrantsTest`, **delivered** |
...
## 12d. Acceptance criteria — Phase B3, accounts and sessions (PD-030), written before the code; rows 1–10 delivered (V023–V026; `AuthTest` 40 + 1, `PasskeyTest` 25, `DbTest`)
