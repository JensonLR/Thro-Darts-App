...
| **Android** | **Scores a match** (PD-081..PD-084): two names, a keypad, a match picked back up by replay, a result that says it is self-reported, the shared Kotlin engine deciding every visit and the shared journal recording it before the screen moves. CI builds and tests it on Linux every push | Everything above the floor: the scoring screen, per-dart entry, a list of past matches, and the chalk grain the board has on iOS. **PD-011's two-player confirmation is there** (PD-085), though nothing on Android can send a result anywhere yet. **Scoring works** (PD-083) and **the journal is the same package the JVM tests run** (PD-082), on the phone, in WAL at synchronous FULL, with its append-only trigger refusing a raw DELETE |
