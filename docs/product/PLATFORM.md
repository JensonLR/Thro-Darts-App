...
| **Android** | **Scores a match** (PD-081, PD-082, PD-083): two names, a keypad, the shared Kotlin engine deciding every visit and the shared journal recording it before the screen moves. CI builds and tests it on Linux every push | Everything above the floor: the scoring screen, per-dart entry, resuming a match in progress, the result screen, and the chalk grain the board has on iOS. **Scoring works** (PD-083) and **the journal is the same package the JVM tests run** (PD-082), on the phone, in WAL at synchronous FULL, with its append-only trigger refusing a raw DELETE |
