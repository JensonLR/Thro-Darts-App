...
- Not looked at in the simulator: ~~the challenge form on another team's front~~ (**looked at 18 September**,
  PD-168 — it was unreachable: the stage had no team you do not run); ~~*Name the match* with matches present~~ and ~~the desk cards in landscape~~ — **both looked at 18 September**
  (PD-169), and correct. Every "never looked at" gap is now closed; three of the four were unreachable rather
  than neglected. `simctl` swipes do not scroll a rotated scene, so a landscape check sees only the fold. **Looked at 18 September** (PD-166): it was unreachable, not unlooked-at — the dev authenticator never named
